...
const STRUCTURED_DATA = {
  ...
  "name": "TracePilot AI",
  "url": "https://tracepilotai.com",
  "applicationCategory": "DeveloperApplication",
  "operatingSystem": "Web",
  "description": "TracePilot lets engineering teams replay, fork, inspect and repair autonomous AI agent workflows in real time. Git-style branching meets runtime observability for LLM agents.",
  "offers": {
    "@type": "Offer",
    "price": "0",
    "priceCurrency": "USD"
  ...
  "keywords": "AI agent debugging, LLM observability, AI workflow tracing, autonomous agent monitoring, OpenAI tracing, LangChain debugging, time-travel debugging"
};
...
const TRUST_PLATFORMS = [
  ...
  { name: 'LangChain', desc: 'Coming soon', isInstallable: false },
  { name: 'CrewAI', desc: 'Coming soon', isInstallable: false }
];
...
const HOW_IT_WORKS_ES = [
  {
    title: '1. Integración Drop-in',
    badge: '1 Línea',
    desc: 'Reemplaza tu llamada a OpenAI o Vercel AI con nuestro wrapper drop-in. Sin refactorización. Observabilidad completa activada al instante en producción.'
  },
  {
  ...
  },
  {
    ...
    badge: null,
    desc: 'Abre trazas fallidas, edita valores en nuestra consola de pruebas interactivas y relanza la ejecución con los datos reparados.'
  }
];

const T = {
  en: {
    heroSub: 'THE CONTROL LAYER FOR AUTONOMOUS AI',
  ...
  es: {
    ...
    integrationStd: 'PAQUETES OFICIALES — INSTALA HOY:',
...
export default function App() {
  ...
  const handleGitHubLogin = async () => {
    ...
    if (error) { console.error('Error logging in:', error.message); setIsAuthenticating(false); return; }
    if (data.url) window.open(data.url, 'github-auth', 'width=600,height=800,left=200,top=100');
  };
  ...
  const handleGoogleLogin = async () => {
    ...
    if (error) { console.error('Error logging in with Google:', error.message); setIsAuthenticating(false); return; }
    if (data.url) window.open(data.url, 'google-auth', 'width=600,height=800,left=200,top=100');
  };
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => { if (session) setupUser(session); });
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === 'SIGNED_IN' && session) setupUser(session);
      if (event === 'SIGNED_OUT') {
        setCurrentUser(null); setIsJoined(false); setTicketData(null);
        setGeneratedApiKey(''); setIsAuthenticating(false);
      }
    });
    const handleFocus = () => {
      supabase.auth.getSession().then(({ data: { session } }) => {
        if (session && !currentUser) setupUser(session);
      });
    };
    window.addEventListener('focus', handleFocus);
    return () => { subscription.unsubscribe(); window.removeEventListener('focus', handleFocus); };
  }, [currentUser]);
  ...
  const handleJoinWaitlist = (e: React.FormEvent) => {
    ...
    setIsJoined(true);
    setTicketData({
      email: emailInput, ticketNum: count,
      timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      boosted: false, provider: undefined, apiKey: undefined
    });
  };
  ...
  return (
    <>
      {/* ─── SEO: Meta tags via next/head ──────────────────────────────────────── */}
      <Head>
        <title>TracePilot AI — Debug AI Agents Like Software</title>
        <meta name="description" content="TracePilot lets engineering teams replay, fork, inspect and repair autonomous AI workflows in real time. Git-style branching meets LLM observability. Free API key for OpenAI & Vercel AI SDK." />
        <meta name="keywords" content="AI agent debugging, LLM observability, AI workflow tracing, autonomous agent monitoring, OpenAI tracing, LangChain debugging, time-travel debugging, AI agent tools, LLM devtools, tracepilot" />
        <meta name="author" content="TracePilot AI" />
        <link rel="canonical" href="https://tracepilotai.com" />

        {/* Open Graph */}
        <meta property="og:type" content="website" />
        <meta property="og:url" content="https://tracepilotai.com" />
        <meta property="og:title" content="TracePilot AI — Debug AI Agents Like Software" />
        <meta property="og:description" content="Replay, fork, inspect and repair autonomous AI agent workflows in real time. Drop-in packages for OpenAI & Vercel AI SDK. Free API key." />
        <meta property="og:image" content="https://tracepilotai.com/og-image.png" />
        <meta property="og:site_name" content="TracePilot AI" />

        {/* Twitter Card */}
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:site" content="@tracepilotai" />
        <meta name="twitter:title" content="TracePilot AI — Debug AI Agents Like Software" />
        <meta name="twitter:description" content="Replay, fork and repair autonomous AI agent workflows in real time. Drop-in npm packages for OpenAI & Vercel AI SDK." />
        <meta name="twitter:image" content="https://tracepilotai.com/og-image.png" />
      ...
      <div className="min-h-screen bg-[#030303] text-[#f5f5f5] font-sans antialiased selection:bg-cyan-500/30 selection:text-white">
        ...
        <header className="sticky top-0 z-50 bg-[#030303]/85 backdrop-blur-md border-b border-neutral-900">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            ...
            <div className="flex items-center space-x-9">
              <a href="#" className="h-16 flex items-center relative overflow-hidden" aria-label="TracePilot AI — Home">
              ...
              <nav className="hidden md:flex items-center space-x-8 text-xs font-semibold text-neutral-400 font-mono tracking-wider uppercase" aria-label="Main navigation">
            ...
            <div className="hidden md:flex items-center space-x-4">
              {currentUser ? (
                <div className="flex items-center space-x-2.5 bg-neutral-900 border border-neutral-800 rounded-full pl-2 pr-3 py-1 text-xs">
                  ...
                  <button 
                    onClick={() => { const supabase = createClient(); supabase.auth.signOut(); }} 
                ...
                <>
                  <button onClick={() => setAuthModalOpen(true)} className="text-xs text-neutral-400 hover:text-white transition font-semibold cursor-pointer">
                    {d.navSignIn}
                  </button>
                  <a href="#waitlist" className="bg-neutral-900 border border-neutral-800 hover:border-neutral-700 text-neutral-200 text-xs px-3.5 py-1.5 rounded-lg font-semibold transition animate-pulse">
                    {d.ctaGetStarted}
                  </a>
                </>
              )}
            ...
            <div className="md:hidden flex items-center">
              <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="text-neutral-400 hover:text-white p-1 cursor-pointer" aria-label="Toggle menu">
          ...
          {mobileMenuOpen && (
            <div className="md:hidden bg-[#040404] border-b border-neutral-900 px-4 py-4 space-y-3 font-mono text-xs uppercase tracking-wider">
              <a href="#problem" onClick={() => setMobileMenuOpen(false)} className="block text-neutral-400 hover:text-white py-1">{d.navProblem}</a>
              <a href="#time-travel" onClick={() => setMobileMenuOpen(false)} className="block text-neutral-400 hover:text-white py-1">{d.navTimeTravel}</a>
              <a href="#how-it-works" onClick={() => setMobileMenuOpen(false)} className="block text-neutral-400 hover:text-white py-1">{d.navHowItWorks}</a>
              <a href="#sdk" onClick={() => setMobileMenuOpen(false)} className="block text-neutral-400 hover:text-white py-1">{d.navSdk}</a>
              <a href="#waitlist" onClick={() => setMobileMenuOpen(false)} className="block text-cyan-400 hover:text-cyan-300 py-1">{d.navPrivateBeta}</a>
              <div className="pt-3 border-t border-neutral-850 flex items-center justify-between">
                {currentUser ? (
                  ...
                  <>
                    ...
                    <a href="#waitlist" onClick={() => setMobileMenuOpen(false)} className="bg-cyan-500 text-black px-3.5 py-1.5 rounded font-bold">{d.ctaGetStarted}</a>
                  </>
                )}
              </div>
            </div>
        ...
        {/* ─── SECTION 1 — HERO ────────────────────────────────────────────────── */}
        <section className="relative pt-12 md:pt-20 pb-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" aria-label="Hero">
        ...
        <section id="problem" className="border-t border-neutral-900 bg-neutral-950/35 py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative" aria-labelledby="problem-heading">
          ...
          <div className="text-center max-w-3xl mx-auto mb-16 space-y-3">
            ...
            <h2 id="problem-heading" className="text-3xl sm:text-4xl font-bold font-display text-white tracking-tight">
              {d.probTitle}
            </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {PROBLEM_CARDS.map((card, index) => {
              ...
              return (
                <div key={card.error} className="bg-neutral-950 border border-neutral-900 hover:border-neutral-800 rounded-xl p-5 text-left flex flex-col justify-between h-[230px] transition relative group overflow-hidden">
                  ...
                  <div>
                    ...
                    <h3 className="text-xs font-bold text-red-400 font-mono tracking-wider block uppercase mb-2">{displayError}</h3>
          ...
          <div className="mt-12 bg-neutral-950 border border-neutral-900 p-6 rounded-xl flex flex-col md:flex-row items-center justify-between gap-4 text-left">
            <p className="text-xs sm:text-sm text-neutral-300 max-w-2xl font-sans">{d.probConclusion}</p>
            <a href="#time-travel" className="text-xs font-semibold font-mono text-cyan-400 hover:text-cyan-300 flex items-center space-x-1 flex-shrink-0 group whitespace-nowrap">
        ...
        <section className="py-16 border-t border-neutral-900 bg-[#050505]/20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" aria-label="Before and after TracePilot">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto text-left">
            <div className="border border-neutral-900 bg-neutral-950/40 p-6 rounded-xl relative overflow-hidden group">
              ...
              <ul className="space-y-4 text-xs font-sans text-neutral-400">
                <li className="flex items-start space-x-3">
                  ...
                  <span className="leading-relaxed">{lang === 'es' ? 'Pasa 3 horas buscando en registros caóticos de terminales de servidor intentando adivinar por qué se atascó un bucle de agentes.' : 'Spend 3 hours digging through chaotic logs across server terminals trying to guess why an agent loop stalled.'}</span>
                </li>
                <li className="flex items-start space-x-3">
                  <span className="text-red-500 font-bold font-mono mt-0.5">✕</span>
                  <span className="leading-relaxed">{lang === 'es' ? 'Reinicia todo el flujo del agente desde el paso uno, desperdiciando miles en credenciales de API y tokens de modelo.' : 'Restart the entire multi-agent workflow from step one, wasting thousands of API credentials and model tokens.'}</span>
                </li>
                <li className="flex items-start space-x-3">
                  <span className="text-red-500 font-bold font-mono mt-0.5">✕</span>
                  <span className="leading-relaxed">{lang === 'es' ? 'Despliega parches especulativos a ciegas, esperando haber resuelto alucinaciones de memoria sin herramientas estándar de validación.' : 'Deploy speculative hotfixes blindly, hoping you solved memory hallucinations without standard validation tools.'}</span>
                </li>
              </ul>
            </div>
            <div className="border border-cyan-950/30 bg-neutral-950/60 p-6 rounded-xl relative overflow-hidden group shadow-lg shadow-cyan-950/5">
              <div className="absolute top-4 right-4 font-mono text-[9px] text-emerald-400 bg-emerald-950/40 border border-emerald-950 px-2 py-0.5 rounded font-bold uppercase tracking-wider">TracePilot AI</div>
              <h3 className="text-base font-bold font-display text-white mb-4">{lang === 'es' ? 'Después de TracePilot' : 'After TracePilot'}</h3>
              <ul className="space-y-4 text-xs font-sans text-neutral-400">
                <li className="flex items-start space-x-3">
                  <span className="text-emerald-400 font-bold font-mono mt-0.5">✓</span>
                  <span className="leading-relaxed text-neutral-200">{lang === 'es' ? 'Reproduce, inspecciona e aísla ejecuciones fallidas de agentes de IA de forma instantánea en 20 segundos.' : 'Replay, inspect, and isolate failed agent executions instantly in 20 seconds.'}</span>
                </li>
                <li className="flex items-start space-x-3">
                  <span className="text-emerald-400 font-bold font-mono mt-0.5">✓</span>
                  <span className="leading-relaxed text-neutral-200">{lang === 'es' ? 'Bifurca el nodo defectuoso exacto dentro de un sandbox local, reescribe los datos de entrada y continúa ejecutando.' : 'Fork the exact failing node into a localized sandbox, rewrite the inputs, and play forward.'}</span>
                </li>
                <li className="flex items-start space-x-3">
                  <span className="text-emerald-400 font-bold font-mono mt-0.5">✓</span>
                  <span className="leading-relaxed text-neutral-200">{lang === 'es' ? 'Prueba correcciones con total seguridad y reubica indicaciones correctas en archivos de plantilla al instante.' : 'Test corrections safely and deploy green path prompts back to template files instantly.'}</span>
                </li>
              </ul>
            </div>
          </div>
        ...
        {/* ─── SECTION 3 — TIME-TRAVEL DEBUGGING ───────────────────────────────── */}
        <section id="time-travel" className="py-20 bg-neutral-900/10 border-t border-neutral-900 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" aria-labelledby="timetravel-heading">
          <div className="text-center max-w-2xl mx-auto mb-16 space-y-3">
            ...
            <h2 id="timetravel-heading" className="text-3xl sm:text-4xl font-bold font-display text-white tracking-tight">
              {lang === 'es' ? 'Detén Fallos con Depuración de Rebobinado Temporal' : 'Stop Failures with Time-Travel Debugging'}
            </h2>
            <p className="text-sm text-neutral-400 font-sans">
              {lang === 'es' ? 'Reproduce cualquier ejecución de IA desde el paso exacto donde falló. Edita prompts, salidas de herramientas o estados de memoria, y relanza al instante.' : 'Replay any AI execution from the exact step where it failed. Edit prompts, tool outputs or memory state, then rerun instantly.'}
            </p>
          </div>
        ...
        <section id="how-it-works" className="py-20 border-t border-neutral-900 bg-neutral-950/20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative" aria-labelledby="howitworks-heading">
          ...
          <div className="text-center max-w-2xl mx-auto mb-16 space-y-3">
            ...
            <h2 id="howitworks-heading" className="text-3xl sm:text-4xl font-bold font-display text-white tracking-tight">
              {d.howTitle}
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {HOW_IT_WORKS.map((work, idx) => {
              ...
              const displayBadge = lang === 'es' ? esWork.badge : work.badge;
              return (
                <div key={work.title} className="bg-[#050505] border border-neutral-900 rounded-xl p-6 text-left hover:border-neutral-800 transition relative overflow-hidden group">
                  ...
                  <h3 className="text-base font-bold text-white mb-2 font-display">{displayTitle}</h3>
                  <p className="text-xs text-neutral-400 leading-relaxed font-sans">{displayDesc}</p>
          ...
          <div className="mt-12 bg-[#050505] border border-neutral-900 rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3 border-b border-neutral-900 bg-neutral-950/60">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-[10px] font-mono text-neutral-500 ml-2">tracepilot-integration.ts</span>
              </div>
              <div className="flex items-center space-x-2">
                <span className="text-[9px] font-mono text-emerald-400 bg-emerald-950/40 border border-emerald-900/30 px-2 py-0.5 rounded font-bold">
                  {lang === 'es' ? 'DROP-IN REPLACEMENT' : 'DROP-IN REPLACEMENT'}
                </span>
              </div>
            </div>
            <div className="p-5 font-mono text-xs leading-relaxed overflow-x-auto">
              ...
              <div className="mb-4">
                ...
                <pre className="text-neutral-600 text-[11px] leading-relaxed select-none">{`const { result } = await tp.wrapOpenAI(async () => {
    ...
    messages: prompt
              ...
              <div>
                ...
                <pre className="text-[11px] leading-relaxed">
                  ...
                  <span className="text-neutral-300"> {'{ traceOpenAI }'} </span>
                  <span className="text-cyan-400">from</span>
                  <span className="text-emerald-400"> 'tracepilot-openai'</span>
                  <span className="text-neutral-300">;</span>
                  {'\n\n'}
                  <span className="text-neutral-500">{'// Replace openai.chat.completions.create → traceOpenAI'}</span>
                  {'\n'}
                  <span className="text-cyan-400">const</span>
                  <span className="text-neutral-300"> response = </span>
                  <span className="text-cyan-400">await</span>
                  <span className="text-yellow-300"> traceOpenAI</span>
                  <span className="text-neutral-300">(tp, openai, {'{'}</span>
                  {'\n'}
                  <span className="text-neutral-300">{'  '}model: </span>
                  <span className="text-emerald-400">"gpt-4o"</span>
                  <span className="text-neutral-300">,</span>
                  {'\n'}
                  <span className="text-neutral-300">{'  '}messages: [{'{ '}role: </span>
                  <span className="text-emerald-400">"user"</span>
                  <span className="text-neutral-300">, content: </span>
                  <span className="text-emerald-400">"Debug my agent"</span>
                  <span className="text-neutral-300"> {'}'}]</span>
                  {'\n'}
                  <span className="text-neutral-300">{'}'});</span>
                  {'\n\n'}
                  <span className="text-neutral-500">{'// Vercel AI SDK — equally simple:'}</span>
                  {'\n'}
                  <span className="text-cyan-400">import</span>
                  <span className="text-neutral-300"> {'{ traceVercel }'} </span>
                  <span className="text-cyan-400">from</span>
        ...
        <section className="py-20 border-t border-neutral-900 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative" aria-label="Live debugging visualization">
        ...
        <section id="sdk" className="py-20 border-t border-neutral-900 bg-[#050505]/45 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative" aria-labelledby="sdk-heading">
          ...
          <div className="text-center max-w-2xl mx-auto mb-16 space-y-3">
            ...
            <h2 id="sdk-heading" className="text-3xl sm:text-4xl font-bold font-display text-white tracking-tight">{d.dxTitle}</h2>
            <p className="text-sm text-neutral-400 font-sans">{d.dxDesc}</p>
          </div>
          <div className="w-full relative z-10">
        ...
        <section id="waitlist" className="py-24 border-t border-neutral-900 bg-[#050505]/40 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative" aria-labelledby="waitlist-heading">
          <div className="absolute inset-0 bg-dot-pattern opacity-10 pointer-events-none" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
            <div className="lg:col-span-6 space-y-6 text-left">
              ...
              <h2 id="waitlist-heading" className="text-4xl sm:text-5xl font-bold font-display text-white leading-tight tracking-tight">
                {d.portalHeader}
              </h2>
              <p className="text-sm sm:text-base text-neutral-400 leading-relaxed font-sans max-w-xl">{d.portalDesc}</p>
              ...
              {isAuthenticating ? (
                <div className="space-y-4 max-w-md">
                  <div className="bg-cyan-950/10 border border-cyan-500/20 p-4 rounded-xl flex items-center space-x-3 text-left">
                    ...
                    <div>
                      <h5 className="text-xs font-bold text-neutral-200 font-sans">{lang === 'es' ? 'Verificando autenticación...' : 'Verifying authentication...'}</h5>
                      <p className="text-[11px] text-neutral-400 font-sans mt-0.5">{lang === 'es' ? 'Espera un momento mientras confirmamos tu identidad en el popup.' : 'Please wait a moment while we confirm your identity in the popup.'}</p>
                    </div>
                  </div>
                </div>
              ) : !isJoined ? (
                <div className="space-y-4 max-w-md">
                  ...
                  <p className="text-[10px] text-neutral-500 font-mono leading-relaxed max-w-sm">
                    {lang === 'es' ? 'No se permiten registros anónimos. Requiere una cuenta instantánea y segura para evitar registros automatizados.' : 'Anonymous submissions are restricted. Secure OAuth login is required to prevent automated registration.'}
                  </p>
                </div>
              ) : (
                <div className="space-y-3 max-w-md">
                  <div className="bg-emerald-950/15 border border-emerald-900/45 p-4 rounded-xl flex items-start space-x-3 text-left">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-neutral-400 font-sans mt-0.5">{d.portalSuccessDesc}</p>
                    </div>
                  </div>
                </div>
              )}
                  ...
                  <div className="p-6 space-y-6">
                    <div className="flex items-center justify-between border-b border-neutral-900 pb-4">
                      <div className="flex items-center space-x-2.5">
                        <div className="w-5 h-5 rounded bg-neutral-900 border border-neutral-800 flex items-center justify-center font-bold text-cyan-400 text-[10px] font-mono">P</div>
                        <div>
                    ...
                    <div className="space-y-4 pt-1 text-left">
                      {(isJoined && (generatedApiKey || ticketData?.apiKey)) ? (
                          ...
                          <div className="p-3.5 bg-neutral-950 border border-cyan-500/25 rounded-lg space-y-2.5 relative overflow-hidden group">
                            ...
                            <div className="flex items-center space-x-2 bg-neutral-900/90 border border-neutral-800 px-2.5 py-2 rounded font-mono text-xs text-[#eee] relative">
                              ...
                              <div className="flex items-center space-x-1.5 shrink-0">
                                <button onClick={() => setShowFullApiKey(!showFullApiKey)} className="text-neutral-500 hover:text-neutral-300 p-1 rounded hover:bg-neutral-855 transition cursor-pointer" title={showFullApiKey ? "Hide Key" : "Show Key"}>
                                  {showFullApiKey ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
                                </button>
                                <button onClick={copyApiKeyToClipboard} className="text-neutral-400 hover:text-cyan-400 p-1 rounded hover:bg-neutral-850 transition cursor-pointer flex items-center" title="Copy API Key">
                            ...
                            <span className="text-[9px] text-neutral-500 font-mono block mt-1 leading-tight">
                              {lang === 'es' ? 'Úsala en tu terminal: export TRACEPILOT_API_KEY="tp_live_••••"' : 'Run in terminal: export TRACEPILOT_API_KEY="tp_live_••••"'}
                            </span>
                          </div>
                          {currentUser && (
                            <div className="bg-cyan-950/10 border border-cyan-500/20 p-3 rounded-lg flex items-center space-x-2.5 text-left text-xs text-neutral-300 font-sans">
                              ...
                              <span>{lang === 'es' ? `Identidad confirmada como ${currentUser.name} (${currentUser.provider}).` : `Identity confirmed as ${currentUser.name} (${currentUser.provider}).`}</span>
                            </div>
                          )}
                          <button 
                            onClick={() => { if (typeof window !== 'undefined') window.location.href = '/dashboard'; }}
                        ...
                        <div className="space-y-3">
                          <p className="text-[10px] font-mono text-neutral-400 leading-normal">{d.passAuthNotice}</p>
                          <div className="grid grid-cols-2 gap-2">
                            <button onClick={handleGitHubLogin} className="bg-neutral-900 hover:bg-neutral-850 text-neutral-250 border border-neutral-800 hover:border-neutral-700 font-medium font-sans text-xs py-2 rounded-lg transition-all flex items-center justify-center space-x-1.5 cursor-pointer">
                              <Github className="w-3.5 h-3.5 text-neutral-300" />
                              <span>{d.passGithubBtn}</span>
                            </button>
                            <button onClick={handleGoogleLogin} className="bg-neutral-900 hover:bg-neutral-850 text-neutral-250 border border-neutral-800 hover:border-neutral-700 font-medium font-sans text-xs py-2 rounded-lg transition-all flex items-center justify-center space-x-1.5 cursor-pointer">
        ...
        <AnimatePresence>
          {authModalOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setAuthModalOpen(false)} className="absolute inset-0 bg-[#020202]/85 backdrop-blur-sm hover:cursor-pointer" />
              <motion.div initial={{ scale: 0.95, y: 15, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: 0.95, y: 15, opacity: 0 }} className="relative bg-[#080808] border border-neutral-800 rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl z-10 p-6 space-y-6" role="dialog" aria-modal="true" aria-labelledby="auth-modal-title">
                <button onClick={() => setAuthModalOpen(false)} className="absolute top-4 right-4 text-neutral-500 hover:text-white transition cursor-pointer" aria-label="Close">
                  <X className="w-4 h-4" />
                </button>
                <div className="text-center space-y-3 pb-2 select-none">
                  <img src="/logo.png" alt="TracePilot AI" className="h-9 mx-auto object-contain" />
                  <p className="text-xs text-neutral-400 max-w-xs mx-auto" id="auth-modal-title">
                    Authenticate your identity to view active telemetry logs, fork execution replays, or claim premium waitlist slots.
                  </p>
                </div>
                <div className="space-y-2">
                  <button onClick={handleGitHubLogin} className="w-full bg-[#111] hover:bg-neutral-900 text-neutral-250 border border-neutral-800 hover:border-neutral-700 hover:cursor-pointer font-semibold py-2.5 rounded-lg text-xs font-sans transition-all flex items-center justify-center space-x-2 active:scale-95">
                    <Github className="w-4 h-4 text-white" />
                    <span>{d.modalGithubBtn}</span>
                  </button>
                  <button onClick={handleGoogleLogin} className="w-full bg-white hover:bg-neutral-100 text-black font-semibold py-2.5 rounded-lg text-xs font-sans transition hover:cursor-pointer active:scale-95 flex items-center justify-center space-x-2">
                ...
                <div className="relative flex py-1 items-center">
                  ...
                  <span className="flex-shrink mx-3 text-[9px] font-mono text-neutral-600 uppercase tracking-widest">Secure Developer Portal</span>
                  <div className="flex-grow border-t border-neutral-900"></div>
                </div>
                <div className="text-center">
                  <p className="text-[10px] text-neutral-500 font-mono">TracePilot respects standard OpenID & OAuth 2.0 specs. We do not store repository or personal tokens.</p>
                </div>
              </motion.div>
            </div>
          )}
        ...
        <AnimatePresence>
          {contactModalOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setContactModalOpen(false)} className="absolute inset-0 bg-[#020202]/85 backdrop-blur-sm hover:cursor-pointer" />
              <motion.div initial={{ scale: 0.95, y: 15, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: 0.95, y: 15, opacity: 0 }} className="relative bg-[#080808]/95 border border-neutral-800 rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl z-10 p-6 space-y-5" role="dialog" aria-modal="true">
                <button onClick={() => setContactModalOpen(false)} className="absolute top-4 right-4 text-neutral-500 hover:text-white transition cursor-pointer" aria-label="Close">
                  <X className="w-4 h-4" />
                </button>
                <div className="text-center space-y-2 select-none">
                  ...
                  <h3 className="text-sm font-bold font-sans text-white tracking-tight">{lang === 'es' ? 'Canales de Soporte' : 'Telemetry Support Channels'}</h3>
                  <p className="text-[11px] text-neutral-500 max-w-xs mx-auto leading-relaxed">{lang === 'es' ? 'Conéctate con nuestro equipo central de ingeniería para resolver dudas o reportar incidentes.' : 'Connect with our core engineering team to resolve questions or report incidents.'}</p>
                </div>
                <div className="space-y-4">
                  <a href="https://discord.gg/ktYCtCA8D" target="_blank" rel="noopener noreferrer" className="w-full bg-[#5865F2] hover:bg-[#4752C4] text-white font-bold py-2.5 rounded-lg text-xs font-sans transition flex items-center justify-center space-x-2 shadow-lg shadow-blue-950/40 select-none cursor-pointer hover:scale-[1.01] active:scale-95">
                  ...
                  <div className="relative flex py-1 items-center">
                    <div className="flex-grow border-t border-neutral-900"></div>
                    <span className="flex-shrink mx-3 text-[9px] font-mono text-neutral-600 uppercase tracking-widest select-none">{lang === 'es' ? 'O POR CORREO' : 'OR VIA EMAIL'}</span>
                    <div className="flex-grow border-t border-neutral-900"></div>
                  </div>
                  <div className="bg-[#040404] hover:bg-[#060606] border border-neutral-850 rounded-lg p-3 text-center transition group">
                    <span className="block text-[10px] text-neutral-500 font-mono mb-1 uppercase tracking-wider select-none">{lang === 'es' ? 'Correo de Contacto' : 'Direct Email Contact'}</span>
                    <a href="mailto:support@tracepilot.ai" className="font-mono text-xs text-cyan-400 hover:text-cyan-300 transition break-all underline select-all">hello@tracepilotai.com</a>
                  </div>
                </div>
                <div className="text-center pt-2 select-none border-t border-neutral-900">
                  <p className="text-[10px] text-neutral-600 font-mono leading-tight">{lang === 'es' ? 'Tiempo de respuesta estimado: < 12 horas' : 'Estimated response latency: < 12 hours'}</p>
                </div>
              </motion.div>
            </div>
          )}
        ...
        <footer className="border-t border-neutral-900 bg-[#020202] py-8 text-neutral-600 text-xs font-mono">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center space-x-2">
              <img src="/logo.png" alt="TracePilot AI" className="h-15 object-contain" />
              <span className="text-neutral-800">|</span>
              <span>{d.footerCopyright}</span>
            </div>
            <nav className="flex items-center space-x-6 text-[11px] flex-wrap justify-center font-sans" aria-label="Footer navigation">
              <a href="/terms" className="hover:text-neutral-300 transition">{lang === 'es' ? 'Términos' : 'Terms'}</a>
              <a href="/privacy" className="hover:text-neutral-300 transition">{lang === 'es' ? 'Privacidad' : 'Privacy'}</a>
              <button onClick={() => setContactModalOpen(true)} className="hover:text-neutral-300 transition cursor-pointer text-[11px]">{lang === 'es' ? 'Contacto' : 'Contacts'}</button>
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="flex items-center bg-neutral-950 border border-neutral-850 p-0.5 rounded-lg">
                <button onClick={() => setLang('en')} className={`px-2 py-1 text-[9px] font-mono tracking-wider uppercase font-bold rounded transition-all cursor-pointer ${lang === 'en' ? 'bg-cyan-500/15 text-cyan-400 border border-cyan-500/20' : 'text-neutral-500 hover:text-neutral-300'}`}>EN</button>
                <button onClick={() => setLang('es')} className={`px-2 py-1 text-[9px] font-mono tracking-wider uppercase font-bold rounded transition-all cursor-pointer ${lang === 'es' ? 'bg-cyan-500/15 text-cyan-400 border border-cyan-500/20' : 'text-neutral-500 hover:text-neutral-300'}`}>ES</button>
              </div>
            </div>
          </div>
        </footer>
